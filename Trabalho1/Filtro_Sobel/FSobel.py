    ...
    for x in range(0, larg):
        if x>0 and y>0 and x<(larg-1) and y<(alt-1):
            Gx = (((-1)*int(img_in[y-1][x-1]))+(0*int(img_in[y-1][x]))+int(img_in[y-1][x+1])
                    +((-2)*int(img_in[y][x-1]))+(0*int(img_in[y][x]))+(2*int(img_in[y][x+1]))
                    +((-1)*int(img_in[y+1][x-1]))+(0*int(img_in[y+1][x]))+int(img_in[y+1][x+1]))

            Gy = (((-1)*int(img_in[y-1][x-1]))+((-2)*int(img_in[y-1][x]))+((-1)*int(img_in[y-1][x+1]))
                    +(0*int(img_in[y][x-1]))+(0*int(img_in[y][x]))+(0*int(img_in[y][x+1]))
                    +int(img_in[y+1][x-1])+(2*int(img_in[y+1][x]))+int(img_in[y+1][x+1]))
...
plt.subplot(122).set_title('Imagem após aplicar o filtro de Sobel')
