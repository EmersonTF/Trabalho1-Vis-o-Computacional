...
img_in = cv2.imread('/Users/skoto/Documents/lena_ruido.jfif', 0)
...
for y in range(0, alt):
    for x in range(0, larg):
        if x>0 and y>0 and x<(larg-1) and y<(alt-1):
            matriz_mediana = np.array([int(img_in[y-1][x-1]),int(img_in[y-1][x]),int(img_in[y-1][x+1])
                            ,int(img_in[y][x-1]),int(img_in[y][x]),int(img_in[y][x+1])
                            ,int(img_in[y+1][x-1]),int(img_in[y+1][x]),int(img_in[y+1][x+1])])
            matriz_mediana = np.sort(matriz_mediana)

            img_out[y,x] = matriz_mediana[4]
        else:
            img_out[y,x] = 0
...
plt.subplot(122).set_title('Imagem após aplicar o filtro da mediana')
